// Ruta: ./ListaFixture.tsx
// (Asegúrate de que la ruta a los tipos sea correcta si los mueves)

import React from 'react';

// --- Tipos (copiados de FixturePage.tsx para referencia) ---
interface Club {
  id: number;
  nombre: string;
}
...
interface ListaFixtureProps {
  ...
  // Opcional: onDelete: (id: number) => void;
}

/**
 * Componente para renderizar la lista de partidos en un formato de marcador.
 */
const PartidoRow: React.FC<{ partido: EncuentroAPI }> = ({ partido }) => {
  // Parsea el resultado, ej: "25-20" o "-"
  const parseResultado = (resultado: string): { score1: string; score2: string } => {
    ...
    // Si el formato es inesperado, se muestra tal cual
  ...
  return (
    <div className="partido-row">
      <span className="team-name team-left">{partido.club1.nombre}</span>
      ...
      <span className="team-name team-right">{partido.club2.nombre}</span>
    </div>
  );
};

/**
 * Componente principal de la lista de Fixtures.
 */
const ListaFixture: React.FC<ListaFixtureProps> = ({ fixtures, onEdit }) => {

  /**
   * Agrupa una lista de partidos por su número de jornada.
   */
  const groupPartidosByJornada = (partidos: EncuentroAPI[]) => {
    return partidos.reduce((acc, partido) => {
      const jornada = partido.jornada || 0; // Agrupa los 'sin jornada' en 0
      if (!acc[jornada]) {
        acc[jornada] = [];
      }
  ...
  return (
    <>
      {/* --- ESTILOS PROFESIONALES PARA LA LISTA --- */}
      <style>{`
        .lista-fixture-container {
          width: 100%;
          display: flex;
          flex-direction: column;
          gap: 1.5rem; /* Espacio entre cada tarjeta de fixture */
        }
        ...
        .fixture-item-header {
          ...
          align-items: center;
          padding: 1rem 1.5rem;
          background-color: #f8f9fa; /* Un fondo sutil para el header */
        ...
        /* Estilos del Marcador (PartidoRow) */
        .partido-row {
          display: flex;
          align-items: center;
          justify-content: space-between;
        ...
        .score-container {
          flex: 0 0 130px; /* Ancho fijo para el marcador */
          text-align: center;
          font-size: 1.3rem;
          font-weight: 700;
          color: #1f3c88;
        ...
        .score {
          ...
          width: 45px; /* Espacio para el número */
        }
        .score.winner {
            color: #000; /* Resalta el ganador (opcional) */
            transform: scale(1.1);
        }
        .score-left {
          text-align: right;
      ...
      <div className="lista-fixture-container">
        ...
        {fixtures.map((fixture) => {
          // Agrupa los partidos de este fixture por su jornada
            ...
            <div key={fixture.id} className="fixture-item-card">
              ...
              <div className="fixture-item-body">
                {Object.keys(jornadasAgrupadas).length === 0 ? (
                  ...
                  // Mapea sobre cada jornada agrupada
