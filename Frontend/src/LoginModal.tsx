...
export default function LoginModal({ onLoginSuccess, onClose }: LoginModalProps) {
  ...
  const [username, setUsername] = useState(""); // Se usa para login y para email de recuperación
  ...
  const handleSubmitLogin = async (e: React.FormEvent) => {
    ...
    // Lógica de login (hardcodeada como pediste)
  ...
  const handleSubmitForgot = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    // Simula una llamada al backend para enviar el email
    // Aquí iría tu fetch POST a /auth/forgot-password
  ...
  /**
   * Cambia de vista y limpia los errores y campos.
   */
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .modal-card form { display: flex; flex-direction: column; gap: 1rem; } /* Gap ajustado */
